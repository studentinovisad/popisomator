...
func New() *http.ServeMux {
	...
	mux.Handle("PATCH /users/me/password", middleware.Chain(
		middleware.RequireAuth,
		middleware.Handle(controller.ChangePassword),
	))
	...
	mux.Handle("PATCH /users/{id}/password", middleware.Chain(
		middleware.RequireAuth,
		middleware.RequireRoles("admin"),
		middleware.Handle(controller.SetUserPassword),
	))
	...
	mux.Handle("GET /item-requests/preparation-report", middleware.Chain(
		middleware.RequireAuth,
		middleware.RequireRoles("manager", "admin"),
		middleware.Handle(controller.GetItemRequestPreparationReport),
	))
	...
	mux.Handle("GET /audit-log", middleware.Chain(
		middleware.RequireAuth,
		middleware.RequireRoles("admin"),
		middleware.Handle(controller.ListAuditLog),
	))
	mux.Handle("GET /audit-log/actors", middleware.Chain(
		middleware.RequireAuth,
		middleware.RequireRoles("admin"),
